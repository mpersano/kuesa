        ...
        const QDir &target) -> Export
{
    ...
    if (m_conf.meshCompressionEnabled()) {
        DracoExportPass pass(source, target, rootObject, m_conf, *m_context);
        rootObject = pass.compress();
        compressedBufferFilename = pass.compressedBufferFilename();
        if (!compressedBufferFilename.isEmpty()) {
            qDebug() << "Compressed buffer filename: " << compressedBufferFilename;
    ...
    case GLTF2ExportConfiguration::Embed::None: {
        ...
        {
            SeparateExportPass pass(target);
            pass.separateURIsInArray(rootObject, GLTF2Import::KEY_BUFFERS);
            pass.separateURIsInArray(rootObject, GLTF2Import::KEY_IMAGES);
